...
const CharacterCreationForm = () => {
  ...
  const getAIResponse = async (message, character) => {
    if (character.useFinetuning) {
      return `[Fine-tuned ${character.name}]: This would be a response from the fine-tuned model.`;
    } else {
      try {
        const response = await axios.post('http://<YOUR_BACKEND_API_URL>/api/contextual-adaptation', {
          character,
          message,
        });
    ...
    <div style={styles.container}>
      ...
      <form onSubmit={handleSubmit} style={styles.form}>
        ...
        <div style={styles.switchContainer}>
          <span style={styles.switchLabel}>Public</span>
          <ToggleSwitch
            checked={character.isPublic}
            onChange={() => setCharacter((prev) => ({ ...prev, isPublic: !prev.isPublic }))}
          />
        </div>
        <div style={styles.switchContainer}>
          <span style={styles.switchLabel}>Use Fine-tuning</span>
          <ToggleSwitch
            checked={character.useFinetuning}
            onChange={() => setCharacter((prev) => ({ ...prev, useFinetuning: !prev.useFinetuning }))}
          />
        </div>
